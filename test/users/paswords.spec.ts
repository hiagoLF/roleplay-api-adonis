...
test.group('Password', (group) => {
  ...
  test('it should return 422 when requires data is not provided or data is invalid', async (assert) => {
    Mail.trap(() => {})
  ...
  test('it should be able to reset password', async (assert) => {
    ...
    await supertest(BASE_URL)
      ...
      .expect(204)
    await user.refresh()
    const checkPassword = await Hash.verify(user.password, newPassword)
